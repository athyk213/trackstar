...
# User parameters
input_dir = "videos/demo"
output_dir = f"infractions-{input_dir.split('/')[-1]}"
...
history = 500
varThreshold = 75
min_area = 75  # Minimum contour area
top_crop = 0
bottom_crop = 0
...
video_files = sorted(
    glob(os.path.join(input_dir, "**", "*.[mM][pP]4"), recursive=True) +
    glob(os.path.join(input_dir, "**", "*.[mM][oO][vV]"), recursive=True)
)
total_frames = 0
for video_path in video_files:
    ...
    print(f"Processing {video_path} -> {vid_output_dir}")
    ...
    while True:
        ...
        fgMask = (fgMask == 255).astype('uint8')*255
        ...
        roi_info = []     # will hold metadata like center coordinates (cx, cy)
        ...
        if roi_images:
            ...
            for idx, result in enumerate(results):
                # Assuming each result has a .probs.top1 attribute returning the predicted class
...
print(f"Processed {total_frames // frame_skip} frames in {total_time:.2f} seconds "
      f"({total_frames // frame_skip / total_time:.2f} FPS)")
