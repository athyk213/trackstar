...
for video_path in video_files:
    ...
    while True:
        ...
        for i in range(len(lane_contours) - 1):
            ...
            for col, y_row in enumerate(zip(*y_values)):
                ...
                if y1 is not None and y2 is not None:
                    cv2.line(mask, (col, y1), (col, y2), 255, 1)
            # masked_frames.append(cv2.bitwise_and(frame, frame, mask=mask))
            masked_frames.append(cv2.bitwise_and(blurred, blurred, mask=mask))
            # masked_frames.append(cv2.bitwise_and(dilated,dilated, mask=mask))
        ...
        for idx, lane_frame in enumerate(masked_frames):
            ...
            cv2.accumulateWeighted(lane_frame.astype(np.float32), avg_frames[idx], weight)  # Ensure consistent data type
            diff = cv2.absdiff(cv2.convertScaleAbs(avg_frames[idx]), lane_frame)
            ...
            # Perform morphological closing to fill small gaps in the contours

            # Detect contours based on area
            # Apply edge detection
            edges = cv2.Canny(dilated, 500, 600)
